...
export class AnthropicLLMProvider extends BaseLLMProvider {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 529 || errorMessage.toLowerCase().includes('overloaded')) {
          throw new Error(
            'Anthropic servers are currently overloaded. Please try again in a few moments.',
          );
        }
  ...
  async loadModels(_apiKey: string): Promise<LLMModel[]> {
    // Anthropic doesn't have a models endpoint yet, but we can update this list periodically
    // For now, return the latest known models
    const knownModels: LLMModel[] = [
      {
        id: 'claude-3-5-sonnet-20241022',
        name: 'Claude 3.5 Sonnet',
    ...
    // Update the internal models list
